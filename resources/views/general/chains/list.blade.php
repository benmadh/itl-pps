        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    <div class="row">
                        <div class="col-md-10 col-sm-10 col-xs-10">
                            <h2>@lang('chains.formName')</h2>
                        </div> 
                        <div class="col-md-2 col-sm-2 col-xs-2">
                            <h2><a href="{{route('chains.create')}}" class="btn btn-primary btn-xs" title="Create Record"><i class="fa fa-plus"></i> @lang('general.app.create_new') </a></h2>
                ...
                <div class="form-group">
                    <form method="post" action="{{ route('chains.search') }}" data-parsley-validate class="form-horizontal form-label-left">
                        {{ csrf_field() }}
                                        ...
                                        <div class="row">                            
                                            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">             
                                                ...
                                                <div class="col-md-8 col-sm-8 col-xs-8">
                                                    <input type="text" value="{{ Request::old('name') ?: '' }}" id="name" name="name" placeholder="Exp: Victoria Secret" class="form-control col-md-7 col-xs-12">
                                                </div>
                                            </div>                                                                
                                        </div>
                                    </div>
                        ...
                        <tbody>
							@if (count($dataLst))								
                                @foreach($dataLst as $row)
                                    <tr>                               
                                        <td>                                   
                                            <a href="{{ route('chains.edit', $row->id) }}" class="btn btn-info btn-xs"><i class="fa fa-pencil" title="Edit Record"></i> </a>
                                            <a href="{{ route('chains.show', $row->id) }}" class="btn btn-danger btn-xs"><i class="fa fa-trash-o" title="Delete Record"></i> </a>
                                        </td>                               
                                        <td>{{$row->name}}</td>                                                                       
