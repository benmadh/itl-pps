    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                ...
                <div class="x_content">
                    <br />
                    <form method="post" action="{{ route('holdmachines.update', $dataObj->id) }}" data-parsley-validate class="form-horizontal form-label-left">
                        ...
                        <div class="form-group{{ $errors->has('date') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <div class="input-group date">
                                    <input type="text" value="{{ Request::old('date') ?: $dataObj->date }}" id="date" name="date" class="form-control col-md-7 col-xs-12">
                                    @if ($errors->has('date'))
                                ...
                                <select class="js-example-basic-single form-control" id="department_id" name="department_id" onchange="getMachineList()">
                                    @if(count($dataDepts))                                        
                                        @foreach($dataDepts as $row)                                                     
                                              <option value="{{$row->id}}" {{ $row->id == $dataObj->department_id ? 'selected="selected"' : ''}}> {{$row->name}}</option>
                                        @endforeach
                                    @endif
                                </select>  
                        ...
                        <div class="form-group{{ $errors->has('machine_id') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <select class="js-example-basic-single form-control" id="machine_id" name="machine_id">
                                    @if(count($machineLst))                                        
                                        @foreach($machineLst as $row)                                                     
                                              <option value="{{$row->id}}" {{ $row->id == $dataObj->machine_id ? 'selected="selected"' : ''}}> {{$row->machin_number}}</option>
                                        @endforeach
                                    @endif
                                </select>  
                                ...
                                <select class="js-example-basic-single form-control" id="shift" name="shift">
                                    @if(count($shiftList))                                        
                                        @foreach($shiftList as $row)                                                     
                                              <option value="{{$row}}" {{ $row == $dataObj->shift ? 'selected="selected"' : ''}}> {{$row}}</option>
                                        @endforeach
                                    @endif
                                </select>  
                                 @if ($errors->has('shift'))
                                ...
                                <select class="js-example-basic-single form-control" id="mc_hold_reason_id" name="mc_hold_reason_id">
                                    @if(count($dataReason))                                        
                                        @foreach($dataReason as $row)                                                     
                                              <option value="{{$row->id}}" {{ $row->id == $dataObj->mc_hold_reason_id ? 'selected="selected"' : ''}}> {{$row->name}}</option>
                                        @endforeach
                                    @endif
                                </select>  
