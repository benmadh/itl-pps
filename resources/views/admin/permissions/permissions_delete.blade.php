    ...
    <div class="row">
        <div class="col-md-12">
            <div class="x_panel">
                ...
                <div class="x_content">
                    <p>@lang('general.app.confirm.delete.question') <strong>{{$permissions->name}}?</strong></p>

                    <form method="POST" action="{{ route('permissions.destroy', ['id' => $permissions->id]) }}">
                        <input type="hidden" name="_token" value="{{ Session::token() }}">
                        <input name="_method" type="hidden" value="DELETE">
                        <button type="submit" class="btn btn-danger">@lang('general.form.delete') <strong>{{$permissions->name}}</strong></button>
                    </form>
                </div>
            </div>
        </div>
