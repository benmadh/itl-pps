    ...
    <div class="row">
        <div class="col-md-12">
            <div class="x_panel">
                ...
                <div class="x_content">
                    <p>@lang('general.app.confirm.delete.question') <strong>{{$roles->name}}?</strong></p>
                    <form method="POST" action="{{ route('roles.destroy', ['id' => $roles->id]) }}">
                        <input type="hidden" name="_token" value="{{ Session::token() }}">
                        <input name="_method" type="hidden" value="DELETE">
                        <button type="submit" class="btn btn-danger">@lang('general.form.delete') <strong>{{$roles->name}}</strong></button>
                    </form>
                </div>
            </div>
        </div>
