                ...
                <div class="x_title">
                    <div class="row">
                        ...
                        <div class="col-md-1 col-sm-1 col-xs-1">
                            <h2> <a href="{{route('dbRotary.index')}}" class="btn btn-info btn-xs"><i class="fa fa-chevron-left"></i> @lang('general.nav.back') </a></h2>
                        </div>
                    </div>
                        ...
                        <div class="row">                       
                            <div class ="col-md-6">
                                <div class="x_panel">                                 
                                    ...
                                    <div class="form-group"> 
                                        <label class="control-label col-md-4 col-sm-4 col-xs-4" for="scan_wo">Scan WO <span class="required">*</span>
                                        </label>
                                        <div class="col-md-7 col-sm-7 col-xs-7">
                                    ...
                                    <div class="form-group{{ $errors->has('size_id') ? ' has-error' : '' }}">
                                        <label class="control-label col-md-4 col-sm-4 col-xs-4" for="size_id">Size <span class="required">*</span>
                                        </label>

                                        <div class="col-md-7 col-sm-7 col-xs-7">
                                            <select class="js-example-basic-single form-control" id="size_id" name="size_id">
                                                <option value=""></option>                                              
                                            </select>                            
                                        </div>
                                    </div>
                                   ...
                                   <div class="form-group" id="div1"> 
                                        <label class="control-label col-md-4 col-sm-4 col-xs-4" for="quantity">Quantity <span class="required">*</span>
                                        </label>
                                        <div class="col-md-7 col-sm-7 col-xs-7">
                                            <input type="text" value="{{ Request::old('quantity') ?: '' }}" id="quantity" name="quantity" class="form-control col-md-7 col-xs-12">
...
<script>    
    ...
    $('#workorder_no').on('keyup',function(e){
        ...
        if (res !== undefined) { 
            ...
            if ($(this).val()) {
                typingTimer = setTimeout(getPrintLstRotary, doneTypingInterval);
            }
        };        
    });
    ...
    function getSizeBrackDown() {         
        ...
        $.get('{{ url('getSizeBrackDown') }}/'+scan_wo, function(data){            
            if(data.length==0){
                ...
                $("#size_id").empty();
                $('#size_id').append('<option value="all"> All Size</option>');
                $.each(data, function(index, val){ 
                    $('#size_id').append('<option value="'+val+'">'+val+'</option>').selectpicker('size_id').trigger('change');
                });               
                $("#quantity").val(''); 
                $("#quantity").focus().select();
            }            
    ...
    function getPrintLstRotary() {        
        ...
        clearErrorBox('quantity');                    
        $.ajax({
            url:'{{ route('partPrintingScreen.store') }}',
            type:'POST',
            data:formData,
            success:function(data) {   
                ...
                if(data.errors) {
                    ...
                    if(data.errors.quantity){
                        addErrorBox('quantity', data.errors.quantity[0]);                        
                    }                                       
    ...
    function getEmployeeTrnList() {
        ...
        $.ajax({           
            url:'{{ url('getPrintingDetailsScreen') }}',           
            type:'GET',
            success:function(data) {            
                $.each(data, function(index, val){
                    $("#data").append('<tr>');   
