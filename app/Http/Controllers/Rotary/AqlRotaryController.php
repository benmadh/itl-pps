...
class AqlRotaryController extends Controller
{
    ...
    public function index(){   
        ...
        $save_msg=null;       
        $dep_ids=array('12');         
        $title="Update AQL Quantity for PFL Rotary (Complete)";
        ...
        return view('rotary.aqls.aqls')->with($params);
    }
    ...
    public function store(Request $request){
        ...
        if ($workorderheaders==null) { 
           return view('rotary.aqls.aqls')->with($params)->withErrors($validator);
        }
        if ($plData==null) { 
           return view('rotary.aqls.aqls')->with($params)->withErrors($validator);
        }
        ...
        $prdQtyTot=$this->getAqlQty($woArrayList);
        ...
        if(!$size_id_array){  
            ...
            foreach($wohLst as $rowHead){
                ...
                foreach($dataLine  as $dataRowLn) {
                    ...
                    $producedQtySize=$this->getAqlQtySizeWise($wo_id, $l_size_no);
                ...
                if($tot_bal_prd_qty!=0){
                    $tracking_des="AQL completed, quantity : ".$tot_bal_prd_qty."(". $woTotQty."), employee name : ".$operaterDetails->full_name." ( EPF # ".$operaterDetails->epf_no." )"; 
                ...
                $finalQty=$this->getAqlkQtyGen($wo_id);
                if($finalQty>=$wo_qty){
                    $input03['aql_status_id'] = 2;
                    ...
                    if($dataLine){
                        ...
                        if($producedQtySize>=$woQtySize){
                          $bal_prd_qty=0;
                            ...
                            if($bal_prd_qty!=0){
                                $tracking_des="AQL completed for size (".$l_size_no."), quantity : ".$bal_prd_qty."(". $woQtySize.")  employee name : ".$operaterDetails->full_name." ( EPF # ".$operaterDetails->epf_no." )"; 
    ...
    public function getAqlDetailsRotary(){ 
     
      $dep_ids=array('12');      
      $mstDataLst = Aql::where('deleted_at', '=', null)
                            ->whereIn('department_id', $dep_ids)
                            ->orderBy('id', 'DESC')
                            ->limit(20)
                            ->get();
      ...
      foreach($mstDataLst  as $row) {
        ...
        $producedQtySize=$this->getAqlQtySizeWise($wo_id, $size_no);
