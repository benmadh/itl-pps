...
class PflRateFileController extends Controller
{
    use UtilityHelperGeneral;

    public function __construct(){
        $this->middleware('permission:pfl_rate_access_allow');
        $this->middleware('permission:pfl_rate_create', ['only' => ['create', 'store']]);
        $this->middleware('permission:pfl_rate_edit', ['only' => ['edit', 'update']]);
        $this->middleware('permission:pfl_rate_delete', ['only' => ['show', 'delete']]);
    }
    ...
    public function index(){         
        ...
        return view('rotary.pflratefile.list')->with($params);
    }
    ...
    public function create(){  
        ...
        return view('rotary.pflratefile.create')->with($params);
    }
    ...
    public function store(Request $request){
        ...
        // foreach ($request->from_ord_qty_ms as $keyMs => $vMs) { 
        //     foreach ($substrateCategoryData as $row) {
        //         $sb_id_ms=$row->id;
        //         $dataMs = array('from_ord_qty'=> $request->from_ord_qty_ms[$keyMs],                  
        //                   'to_ord_qty'=> $request->to_ord_qty_ms[$keyMs], 
        //                   'batch_size_cat_ref'=> $request->batch_size_cat_ref_ms[$keyMs],
        //                   'substrate_category_id'=> $sb_id_ms,
        //                   'metres'=> $request->$sb_id_ms[$keyMs],                        
        //                   'mrn_rate_file_rotaries_id'=> $mrnRateFile_id,
        //                   'created_at'=> date('Y-m-d H:i:s'),                   
        //                   'created_by'=> Auth::user()->id);  
        //         MachineSpeedRotary::insert($dataMs); 
        //     } 
        // } 

        return redirect()->route('pflratefile.index')->with('success', trans('general.form.flash.created',['name' => $dataObjIns->companies->name]));
    }
    ...
    public function edit($id) {  
        ...
        // $dataMs = DB::table('machine_speed_rotaries')
        ...
        return view('rotary.pflratefile.edit')->with($params);
    }
    ...
    public function update(Request $request, $id){
        ...
        return redirect()->route('pflratefile.index')->with('success', trans('general.form.flash.updated',['name' => $dataObjUpd->companies->name]));
    }
    ...
    public function show($id){
          try{
            $dataObj = MrnRateFileRotary::findOrFail($id);
            $params = [
                'title' => 'Delete Rotary Rates',
                'dataObj' => $dataObj,
            ];
            return view('rotary.pflratefile.delete')->with($params);
    ...
    public function search(Request $request){
        ...
        return view('rotary/pflratefile/list', ['dataLst' => $dataLst, 
                                                'mctData' => $mctData, 
                                                'comData' => $comData, 
                                                'locData' => $locData, 
                                                'tableTypeLst' => $tableTypeLst]);
    ...
    private function doSearchingQuery($constraints){            
        $query = MrnRateFileRotary::where('deleted_at', '=', null)->where('department_id', '=', 12)->orderBy('id', 'DESC');
    ...
    public function edit_machine_speed($id){
        ...
        try{
            ...
            return view('rotary.pflratefile.edit_machine_speed')->with($params);
        }catch (ModelNotFoundException $ex){
            if ($ex instanceof ModelNotFoundException){
                return response()->view('errors.'.'404');
            }
    ...
    public function update_machine_speed(Request $request) {          
        ...
        return redirect()->route('pflratefile.index')->with('success', trans('general.form.flash.updated',['name' => $request->id]));   
    }
