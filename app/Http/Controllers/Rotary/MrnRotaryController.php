...
class MrnRotaryController extends Controller
{
    use UtilityHelperGeneral;

    public function __construct(){
        $this->middleware('permission:mrn_rotary_access_allow');       
    }
    ...
    public function index(){  
        ...
        $labelTypeList = $this->genLabelType(null);
        $dataLst = DB::table('mrn_header_rotaries')
                            ->leftJoin('work_order_headers', 'mrn_header_rotaries.work_order_header_id', '=', 'work_order_headers.id') 
                            ->leftJoin('machine_types', 'mrn_header_rotaries.machine_type_id', '=', 'machine_types.id')
                            ->leftJoin('users', 'mrn_header_rotaries.created_by', '=', 'users.id')                            
                            ->select('mrn_header_rotaries.*', 'machine_types.name as machine_types_name', 'work_order_headers.lenght as lenght','work_order_headers.no_of_colors_front as no_of_colors_front','work_order_headers.no_of_colors_back as no_of_colors_back', 'work_order_headers.size_changes as size_changes', 'users.name as created_by')
        ...
        return view('rotary.mrnrotary.mrnrotary')->with($params);
    }
    ...
    public function store(Request $request){
        ...
        switch ($request->input('action')) {
            case 'print':
                ...
                return view('rotary/mrnrotary/mrnrotary', ['title' => 'M R N For PFL Rotary',
                                                            'searchingVals' => $constraints,
                                                            'dataObjWo' => $dataObjWo,
                                                            'mctData' => $mctData,
                                                            'tableTypeLst' => $tableTypeLst,
            ...
            case 'search':
                ...
                return view('rotary/mrnrotary/mrnrotary', ['dataLst' => $dataLst, 
                                                        'mctData' => $mctData, 
                                                        'dataObjWo' => $dataObjWo,
    ...
    public function edit($id) { 
        ...
        return view('rotary.mrnrotary.view')->with($params);
    }

    private function doSearchingQuery($constraints){  
        $dep_id=12; 

         $query = DB::table('mrn_header_rotaries')
                            ->leftJoin('work_order_headers', 'mrn_header_rotaries.work_order_header_id', '=', 'work_order_headers.id') 
                            ->leftJoin('machine_types', 'mrn_header_rotaries.machine_type_id', '=', 'machine_types.id')
                            ->leftJoin('users', 'mrn_header_rotaries.created_by', '=', 'users.id')                            
                            ->select('mrn_header_rotaries.*', 'machine_types.name as machine_types_name', 'work_order_headers.lenght as lenght','work_order_headers.no_of_colors_front as no_of_colors_front','work_order_headers.no_of_colors_back as no_of_colors_back', 'work_order_headers.size_changes as size_changes', 'users.name as created_by')                            
    ...
    public function print_mrn($id){
        ...
        if($dataObj){
            ...
            $html .= '<tr>            
                          ...
                          <td colspan="2" width="27%" align="right" ><h2>PFL - Rotary ('.($dataObj->table_type).') </h2></td>
                        </tr>';
        ...
        // $html .= '<br pagebreak="true">';  
            ...
            TPDF::Output('Mrn-PFL Rotary-'.$workorder_no, 'D');
            TPDF::reset();
    ...
    public function getWoDetais($main_workorder_id){        
        $mwoList= WorkOrderHeader::where('id', '=', $main_workorder_id)->where('deleted_at', '=', null)->first();
        $status="A";  
        $id=$mwoList->id;
        $wono=$mwoList->workorder_no;       
        $labellenght=$mwoList->lenght;
        $labelwidth=$mwoList->width;
        $no_of_colors_front=$mwoList->no_of_colors_front;
        if(!$no_of_colors_front){
            $no_of_colors_front=0;
        }
        $no_of_colors_back=$mwoList->no_of_colors_back;
        if(!$no_of_colors_back){
            $no_of_colors_back=0;
        }
        $labelreferences_id=$mwoList->label_reference_id;        
        $itemList = DB::table('item_label_references')
                       ->leftJoin('label_references', 'item_label_references.label_reference_id', '=', 'label_references.id')
                       ->leftJoin('items', 'item_label_references.item_id', '=', 'items.id')
                       ->leftJoin('substrate_categories', 'items.substrate_category_id', '=', 'substrate_categories.id')
                       ->select('item_label_references.*', 'items.name as item_name', 'items.id as item_id','substrate_categories.name as sbc_name','substrate_categories.id as sbc_id') 
                       ->where('item_label_references.label_reference_id', '=', $labelreferences_id )
                       ->where('item_label_references.deleted_at', '=', null)
                       ->first();     
        $item_name="Please Update the Item BOM";
        $sbc_name="";
        $item_id="";
        $sbc_id="";
        if($itemList!=null){
            ...
            $sbc_name=$itemList->sbc_name;
            $sbc_id=$itemList->sbc_id;
        }
        
        if($labellenght==null||$labellenght==0){
            if($labelreferences_id!=null){
                $refList = LabelReference::findOrFail($labelreferences_id);                              
                $labellenght=$refList->default_lenght;
                $labelwidth=$refList->default_width;                 
                $status="D";              
            }
        }else{
            $refList = LabelReference::findOrFail($labelreferences_id);            
        ...
        $data=array();
        $wohLst = WorkOrderHeader::where('main_workorder_id', '=', $main_workorder_id)->where('deleted_at', '=', null)->get();
        $wohlst_array_list = array();
        $wo_quantity_tot=0; 
        foreach($wohLst  as $wohRow) {
            $header_id=$wohRow->id;
            $workorderno=$wohRow->workorder_no;
            $deliverydate=$wohRow->mwo_delivery_date;
            $wo_quantity=$wohRow->wo_quantity;

            $wolLst = WorkOrderLine::where('deleted_at', '=', null)                        
                        ->where('work_order_header_id',$header_id)
                        ->get();          
            
            $wol_des=""; 
            foreach($wolLst  as $wolRow) {
                $size_no=$wolRow->size;
                $size_qty=$wolRow->quantity;
                $wo_quantity_tot+=$size_qty;
                $wol_des.=$size_no.'('.$size_qty.') ';
            }
            $wohlst_array_list[]=array('workorderno'=>$workorderno,
                          'deliverydate'=>$deliverydate,
                          'wollst_array_list'=>$wol_des,
                          'wo_quantity'=>$wo_quantity);
        } 

        $data[]=array('labellenght'=>$labellenght,
                          'labelwidth'=>$labelwidth,
                          'no_of_colors_front'=>$no_of_colors_front,
                          'no_of_colors_back'=>$no_of_colors_back,  
                          'item_name'=>$item_name, 
                          'item_id'=>$item_id,
                          'sbc_name'=>$sbc_name,   
                          'sbc_id'=>$sbc_id,                        
                          'wo_quantity_tot'=>$wo_quantity_tot,
                          'woh_des'=>$wohlst_array_list,                          
                          'status'=>$status);        
        if(count($data)){             
             return response($data);
        }else{
            $wono_X="No Result Found";
            $data[]=array('labellenght'=>$wono_X);
            return response($data);
        }       
    }
}
