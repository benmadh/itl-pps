...
use Larashop\Models\General\Packing;
use Larashop\Models\General\OrderTracking;

Use Larashop\Http\Controllers\General\UtilityHelperGeneral;

class PackingRotaryController extends Controller
{
    use UtilityHelperGeneral;
    public function __construct(){
        $this->middleware('permission:packing_rotary_access_allow');
    }  
    ...
    public function index(){   
        ...
        $save_msg=null;       
        $dep_ids=array('12');         
        $title="Update Packing Quantity for PFL Rotary (Complete)";
        ...
        return view('rotary.packing.packing')->with($params);
    }
    ...
    public function store(Request $request){
        ...
        if ($workorderheaders==null) { 
           return view('rotary.packing.packing')->with($params)->withErrors($validator);
        }
        if ($plData==null) { 
           return view('rotary.packing.packing')->with($params)->withErrors($validator);
        }
        ...
        $prdQtyTot=$this->getPackedQty($woArrayList);
        ...
        if(!$size_id_array){  
            ...
            foreach($wohLst as $rowHead){
                ...
                foreach($dataLine  as $dataRowLn) {
                    ...
                    $producedQtySize=$this->getPackingQtySizeWise($wo_id, $l_size_no);
                ...
                if($tot_bal_prd_qty!=0){
                    $tracking_des="Packing completed, quantity : ".$tot_bal_prd_qty."(". $woTotQty."), employee name : ".$operaterDetails->full_name." ( EPF # ".$operaterDetails->epf_no." )"; 
                    ...
                    $ordertrackings = OrderTracking::create([
                        ...
                        'date' => $prd_date, 
                        'department_id' => $wo_dep_id,
                        'tracking_dep'=>"Packing Completed",
                ...
                $finalQty=$this->getPackQtyGen($wo_id);
                if($finalQty>=$wo_qty){
                    $input03['packing_status_id'] = 2;
                    ...
                    if($dataLine){
                        ...
                        if($producedQtySize>=$woQtySize){
                          $bal_prd_qty=0;
                            ...
                            if($bal_prd_qty!=0){
                                $tracking_des="Packing completed for size (".$l_size_no."), quantity : ".$bal_prd_qty."(". $woQtySize.")  employee name : ".$operaterDetails->full_name." ( EPF # ".$operaterDetails->epf_no." )"; 
    ...
    public function getPackingDetailsRotary(){ 
     
      $dep_ids=array('12');      
      $mstDataLst = Packing::where('deleted_at', '=', null)
                            ->whereIn('department_id', $dep_ids)
                            ->orderBy('id', 'DESC')
                            ->limit(20)
                            ->get();
      ...
      foreach($mstDataLst  as $row) {
        ...
        $producedQtySize=$this->getPackingQtySizeWise($wo_id, $size_no);
