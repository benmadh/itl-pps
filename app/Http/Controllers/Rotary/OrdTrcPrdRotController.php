...
class OrdTrcPrdRotController extends Controller
{
    use UtilityHelperGeneral;
    public function __construct(){
        $this->middleware('permission:order_tracking_prd_rotary_access_allow');
    } 
    ...
    public function index(){  
        ...
        $save_msg=null; 
        $params = [
            'title' => 'Scan Work Orders for Production Departments',          
        ];
        return view('rotary.ordtrcprdrot.ordtrcprdrot')->with($params);       
    }
    ...
    public function store(Request $request){      
       try{ 
          ...
          if($dataObj == null){   
            ...
            $tracking_dep="Production";
            if($departments_id=="12"){                
                $tracking_des="Production - PFL Rotary";
            }elseif($departments_id=="13"){
                $tracking_des="Production - Screen";
            }elseif($departments_id=="14"){
                $tracking_des="Production - Thermal";
            }elseif($departments_id=="15"){
                $tracking_des="Production - Offset";
            }elseif($departments_id=="16"){
                $tracking_des="Production - Digital";    
            }elseif($departments_id=="17"){
                $tracking_des="Production - Heat Transfer";
            }elseif($departments_id=="18"){
                $tracking_des="Production - RFID";           
            }else{
