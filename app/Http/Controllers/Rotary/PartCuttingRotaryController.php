...
class PartCuttingRotaryController extends Controller
{
    use UtilityHelperGeneral;
    public function __construct(){
        $this->middleware('permission:cutting_rotary_access_allow');
    }  
    ...
    public function index(){       
        ...
        $save_msg=null;       
        $dep_ids=array('12');         
        $title="Update Cutting Quantity for PFL Rotary (Part)";
        ...
        return view('rotary.partcutting.partcutting')->with($params);
    }
    ...
    public function store(Request $request){
        ...
        if ($workorderheaders==null) { 
           return view('rotary.partcutting.partcutting')->with($params)->withErrors($validator);
        }
        if ($plData==null) { 
           return view('rotary.partcutting.partcutting')->with($params)->withErrors($validator);
        }
        ...
        $prdQtyTot=$this->getWoProduceCutQty($workorderheaders_id); 
        ...
        if($size_id){ 
            $wohLst = WorkOrderHeader::where('id', '=', $workorderheaders_id)->where('deleted_at', '=', null)->first();
            if($wohLst){
                $wo_id=$wohLst->id;
                ...
                if($dataLine){
                    ...
                    $producedQtySize=$this->getCuttingQtySizeWise($wo_id, $size_id);
                    ...
                    if($producedQtySize>=$woQtySize){
                      $bal_prd_qty=0;
                        ...
                        if($quantity!=0){
                            $producedQtySize=$this->getCuttingQtySizeWise($wo_id, $size_id);                            
                            if($producedQtySize>=$woQtySize){
                                $tracking_des="Cutting completed for size (".$size_id."), quantity is : ".$quantity."(". $woQtySize.") Machine : ".$machineDetails->machin_number." Operator : ".$operaterDetails->full_name." ( EPF # ".$operaterDetails->epf_no." )";
                            }else{
                                $tracking_des="Part cutting completed for size (".$size_id."), quantity is : ".$quantity."(". $woQtySize.") Machine : ".$machineDetails->machin_number." Operator : ".$operaterDetails->full_name." ( EPF # ".$operaterDetails->epf_no." )";
                            } 
